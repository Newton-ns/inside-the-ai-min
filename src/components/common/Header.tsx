...
import { UserCircle, Mic, Globe, Scale, Sun, Moon, ChevronDown, LogOut } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenVoiceAssistant, theme = 'dark', onToggleTheme }) => {
  ...
  const labels = {
    ...
    logout: language === 'en' ? 'Sign out' : language === 'hi' ? 'साइन आउट' : 'வெளியேறு',
  ...
  return (
    <header className="classic-header">
      <div className="classic-govbar"><div className="classic-container classic-govbar-inner"><div className="classic-gov-left"><span className="classic-code">SIH-26034</span><span className="classic-dot">•</span><span>{labels.govPlatform}</span><span className="classic-dot">•</span><span>{t.govIndia}</span></div><div className="classic-gov-right"><span className="classic-status"><span className="classic-status-dot" /> {labels.systemActive}</span><div className="classic-language"><Globe size={13} /><select value={language} onChange={(e) => setLanguage(e.target.value as 'en' | 'hi' | 'ta')} aria-label="Language"><option value="en">English</option><option value="hi">हिन्दी</option><option value="ta">தமிழ்</option></select></div></div></div></div>
      <div className="classic-nav"><div className="classic-container classic-nav-inner">
        <Link to="/" className="classic-brand"><div className="classic-mark"><Scale size={23} strokeWidth={2.2} /></div><div><div className="classic-brand-name">LegalMetrology<span>AI</span></div><div className="classic-brand-sub">{labels.brandSub}</div></div></Link>
        <nav className="classic-links" aria-label="Primary navigation">{navLinks.map((link) => { const active = link.to === '/' ? location.pathname === '/' : location.pathname === link.to || (link.to === '/verify' && location.pathname.startsWith('/verify/')); return <Link key={link.to} to={link.to} className={active ? 'active' : ''}>{link.label}</Link>; })}</nav>
        <div className="classic-actions">
          <button onClick={onToggleTheme} className="classic-theme-toggle" title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} aria-label={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}>{theme === 'dark' ? <Sun size={15} /> : <Moon size={15} />}<span>{theme === 'dark' ? labels.light : labels.dark}</span></button>
          ...
          <button onClick={logout} className="classic-logout" title={labels.logout} aria-label={labels.logout}><LogOut size={15} /><span>{labels.logout}</span></button>
        </div>
      </div></div>
